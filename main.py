...
my_email = os.getenv("HEALTHHACKEMAIL")
my_password = os.getenv("HEALTHHACKEMAILPASSWORD")
...
def send_emails(name, contact_email, phone_number, message, my_email, my_password):
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(email=form.email.data, name=form.name.data, password=hashed_and_salted_password, )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash('That email is not registered with us.')
            return redirect(url_for('login'))
...
def contact():
    ...
    if form.validate_on_submit():
        ...
        send_emails(name=name, contact_email=email, phone_number=phone, message=message, my_email=my_email,
...
def add_new_post():
    form = CreatePostForm()
    if form.validate_on_submit():
        new_post = BlogPost(title=form.title.data, subtitle=form.subtitle.data, body=strip_invalid_html(form.body.data),
...
def edit_post(post_id):
    ...
    edit_form = CreatePostForm(title=post.title, subtitle=post.subtitle, img_url=post.img_url, author=current_user,
                               body=post.body)
